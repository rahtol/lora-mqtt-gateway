...
void WebApiMqttClass::onMqttStatus(AsyncWebServerRequest* request)
{
    ...
    root["mqtt_connected"] = MqttSettings.isConnected();
...
void WebApiMqttClass::onMqttAdminPost(AsyncWebServerRequest* request)
{
...

    {
        ...
        if (strcmp(config.Mqtt.Topic, root["mqtt_topic"].as<String>().c_str())) {
            ...
            // TODO: Remove this when we have a better solution for the topic change
            retMsg["message"] = "Changing the base topic is not supported yet. Please use the same topic as before!";
            retMsg["code"] = WebApiError::MqttTopicChangeNotSupported;
            WebApi.sendJsonResponse(request, response, __FUNCTION__, __LINE__);
            return;
        }
    }
